...
/* harmony import */ var _nav_AuthorNav__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(4512);
...
function authorLayout(props) {
    ...
    const getcurrentAuthor = async ()=>{
        try {
            const { data  } = await axios__WEBPACK_IMPORTED_MODULE_8___default().get("/current-author");
            setLoading(false);
        } catch (err) {
            console.log(err);
            router.push("/");
